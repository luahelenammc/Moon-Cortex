...
root = Path('.')
old_module = root / 'modules' / 'finance-bootstrap'
module = root / 'modules' / 'financial-living-system'

if old_module.exists() and not module.exists():
    old_module.rename(module)

old_bootstrap = module / 'FINANCE_BOOTSTRAP_SEED.md'
bootstrap = module / 'ADAPTIVE_FINANCE_BOOTSTRAP.md'
if old_bootstrap.exists() and not bootstrap.exists():
    old_bootstrap.rename(bootstrap)
...
# Advance current module-facing version references without rewriting changelog history.
for path in module.rglob('*.md'):
    if path.name == 'CHANGELOG.md':
        continue
    text = read(path).replace('0.1.0-pre.2', '0.1.0-pre.3')
    write(path, text)

# Correct canonical routes in current contracts.
targeted = [
    root / 'ARCHITECTURE.md',
    root / 'PUBLIC_BOUNDARY.md',
    root / 'LICENSING.md',
    root / 'NOTICE',
    module / 'docs' / 'FINANCIAL_REALITY_TEST.md',
    module / 'docs' / 'PRIVACY_AND_CLAIMS.md',
    bootstrap,
]
for path in targeted:
    text = read(path)
    text = text.replace(
        'modules/finance-bootstrap/FINANCE_BOOTSTRAP_SEED.md',
        'modules/financial-living-system/ADAPTIVE_FINANCE_BOOTSTRAP.md',
    )
    text = text.replace('modules/finance-bootstrap/', 'modules/financial-living-system/')
    text = text.replace('FINANCE_BOOTSTRAP_SEED.md', 'ADAPTIVE_FINANCE_BOOTSTRAP.md')
    write(path, text)

architecture = read(root / 'ARCHITECTURE.md').replace('0.1.0-pre.2', '0.1.0-pre.3')
architecture = architecture.replace(
    'The historical filename `ADAPTIVE_FINANCE_BOOTSTRAP.md` is retained for path stability. Its current semantic role is the **Adaptive Finance Bootstrap** for the Financial Living System.\n',
...
root_readme = '''<!-- SPDX-FileCopyrightText: 2026 Lua Helena Moon Martins Cardoso (Moon) -->
...
pre3_root = '''## 0.1.0-pre.3 — 2026-09-01
...
'''
if '## 0.1.0-pre.3 — 2026-09-01' not in root_changelog:
...
pre3_module = '''## 0.1.0-pre.3 — 2026-09-01
...
'''
if '## 0.1.0-pre.3 — 2026-09-01' not in module_changelog:
...
archive = downloads / 'financial-living-system.zip'
if archive.exists():
    archive.unlink()
