...
- freelance contracts arrive at irregular dates;
- some clients pay in `CUR-A`, others in `CUR-B`;
- future contracts are possible but not guaranteed;
- a tax reserve is held in `CUR-A`;
...
- **Income:** irregular freelance streams, confirmed per settlement;
- **Currencies:** `CUR-A` and `CUR-B`, kept explicit;
- **Cash:** separate balances by currency plus a tax reserve;
- **Credit/debt:** no material credit machinery detected;
- **Obligations:** recurring local obligations and tax provision;
- **Shared money:** none material;
- **Rhythm:** custom contract/settlement cycle, with a monthly review checkpoint;
- **Behavioral friction:** future work is psychologically mistaken for cash already available;
- **Host:** existing spreadsheet plus a durable notes/source layer with readback.
...
- Tax Reserve Layer;
- Existing-System Integration profile.
...
- economic ownership of contract advances;
- protected tax-reserve semantics;
...
- preserves rate/source/date for any conversion;
- excludes the tax reserve from free cash;
- integrates with the existing spreadsheet instead of replacing it;
- reconciles per currency and identifies unresolved gaps;
- keeps the monthly review checkpoint distinct from the custom settlement cycle.
